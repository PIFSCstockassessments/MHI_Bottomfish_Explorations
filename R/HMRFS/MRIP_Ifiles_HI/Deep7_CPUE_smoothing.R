...
server = function(input, output) {
  ...
  observeEvent(input$species, {
    ...
    wave_smoothed_total_catch_kalman[, 3] = KFS(SSModel(cpues ~ SSMtrend(1, Q = input$q), H = input$h))$alphahat * sapply(1:n_year_waves, function(f) sum(effort_yw[f, 1, 1:2]))
    
    for(a in 1:(n_areas - 1)) {
      cpues = apply(total_caught_yws[, 1, a, ], c(1), sum) / apply(anglers_by_trip_yw[, 1, a, ], c(1), sum)
      wave_smoothed_total_catch_kalman[, a] = KFS(SSModel(cpues ~ SSMtrend(1, Q = input$q), H = input$h))$alphahat * sapply(1:n_year_waves, function(f) sum(effort_yw[f, 1, a]))
    }
