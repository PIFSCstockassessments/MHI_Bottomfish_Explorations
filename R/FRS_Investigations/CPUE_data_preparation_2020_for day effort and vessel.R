...
datafull=read.csv("C:\\Users\\John.Syslo\\Documents\\2020_Deep_7\\2020 Data prep files\\CPUE\\ForGroupCpueAnalysis_gear3Step3_2020.csv",header=T)
...
#Single reproting day (CML-FISHED) combo up through Sept. 2002
...
data4b=merge(data4[substr(data4$FISHED,1,7)<="2002-09",],ports2,by="PORT_LAND",all.x=T) 
...
newdata4=data4[substr(data4$FISHED,1,7)>"2002-09",]
...
#Use only for records prior to September 2002
monthday=table(substr(data5[substr(data5$FISHED,1,7)<="2002-09",]$FISHED,6,10))
...
past_data5=unique(data5[substr(data5$FISHED,1,7)<="2002-09",c("trip2","LICENSE","FISHED","deep7_lbs","FYEAR")]) #trips in past
...
records_firstlast=merge(data5[substr(data5$FISHED,1,7)<="2002-09",],trips_firstlast,by=c("trip2","LICENSE","FISHED","deep7_lbs","FYEAR"))
...
dim(neverDeep7) #1482 people never caught a Deep7 
sum(neverDeep7$records) #have 7851 records
####----#####
...
dimnames(data9)[[2]][83]="uku_lbs" #dim was previously 86
####----#####
...
#new line to skip output and input above
...
#Dont use SUBAREA since only apply to 16123 (all are 16123-C)

#DONT subset the data for the day effort/vessel exploration, so comment out this 2 lines
#tdata=fulldata[,c("FYEAR","FISHED","trip2","AREA","n_areas","HOURS","days","SPECIES","lbs","deep7_lbs","fisher","cum_exp","qtr","uku_lbs","speed","xdir","ydir")]
names(fulldata)[77]="n_areas_e" #n_areas prior to Oct. 2002 (early)
...
ordered_d7lhat$trip3=as.numeric(ordered_d7lhat$trip2)+ordered_d7lhat$tripADD/10 #added "as.numeric"
####----#####
...
####----#####
tdata3=tdata2[-which(tdata2$trip3%in%trips_0hr),]
####----#####
#
...
old_tdata3=tdata3 #update so it just pulls all the data here
...
#recent_tdata4=tdata4[tdata4$FISHED>="2002-10-01",] #update for full time perior - below
recent_tdata4=tdata4[tdata4$FISHED>="2018-06-30",] 
...
tdata5$n_areas_all=pmax(tdata5$n_areas_e,tdata5$n_areas_r,na.rm=TRUE) #combine n_areas_e with n_areas_r  #LEFT OFF HERE 
####----#####
#
...
length(tied_trips) #137 in 2020 update
...
##-##
####----#####   #change this so cpue is all based on day effort
tdata6$cpue=NA
tdata6[tdata6$FISHED<"2002-10-01","cpue"]=tdata6[tdata6$FISHED<"2002-10-01","d7catch"]/tdata6[tdata6$FISHED<"2002-10-01","dayeffort"]
tdata6[tdata6$FISHED>="2002-10-01","cpue"]=tdata6[tdata6$FISHED>="2002-10-01","d7catch"]/tdata6[tdata6$FISHED>="2002-10-01","dayeffort"] #changed to day effort
####----#####
#write.csv(tdata6,"C:\\Users\\John.Syslo\\Documents\\2020 Deep 7\\2020 Data prep files\\CPUE\\tdata6_2017.csv",row.names=F) #bad added 5/20/20
write.csv(tdata6,"C:\\Users\\John.Syslo\\Documents\\2023 Deep 7\\Raw FRS depredation\\tdata6_all_day_effort.csv",row.names=F) #write the whole thing out and reduce to unique trips later....while adding vessel
