...
# specify (coarse) model configuration
    ...
    species = "mv"
    data_treatment = "05"
    catchability_covariates = "gearSP12" # vanilla
    abundance_covariates = "depth3.hardness3.complexity4.slope3.islandG" # vanilla
    lehi_filter = TRUE
...
# 1) bring in data
	...
	# define abundance section
		...
		ab1_formula = ~ bs(depth_sc,df=3) + bs(hardness_sc,df=3) + bs(complexity_sc,df=4) + bs(slope_sc,df=3) + islandG
        ab2_formula = ~ 0
