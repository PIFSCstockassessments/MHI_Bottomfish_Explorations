...
# 3) Fit single species (hyqu) model using delta-Gamma distribution
...
# set working directory
	...
	working_dir = paste0(proj.dir,"VAST/model_runs/dgamma_single_hyqu/")
...
# 1) bring in data
	...
	# subset to species
	bfish_df = bfish_combined_long_dt %>% .[species_cd %in% c("hyqu")] %>% as.data.frame(.)
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		fit_setup = fit_model( settings=settings,
	    	  					...
	    	  					c_i = as.numeric(factor(bfish_df[,'species_cd'],levels=c("hyqu")))-1,
	    	  					category_names=c("hyqu"),
	    	  					covariate_data = NULL,
		...
		settings$FieldConfig = matrix( c(0,"IID","IID",0,"IID","IID"), ncol=2, nrow=3, dimnames=list(c("Omega","Epsilon","Beta"),c("Component_1","Component_2")) )
