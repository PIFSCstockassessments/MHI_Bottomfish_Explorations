...
# specify (coarse) model configuration
    ...
    species = "mv"
    data_treatment = "05"
    catchability_covariates = "gearSP.platform" # vanilla
...
# set working directory
    ...
    load_spatial_path = paste0(proj.dir,"VAST/model_runs/2022-12-29/2021_pldg_mv_05_v_v_TRUE_7.5_FALSE_TRUE_pit_xval/")
	# xval path
...
# 1) bring in data
    ...
    # define catchability section
        q_data = bfish_df[,c("year","species_cd","gear_type","platform")]
        ...
        q1_formula = ~ gear_type:category + platform
...
# predicted density
    ...
    # effects plots
        plot_effect1_dt.list = as.list(rep(NA,length(qeffectnames_1)))
        for(i in 1:length(qeffectnames_1))
        {
            tmp_cols = qeffectnames_1[i]
            tmp_pe_dt = qeffect_1_dt[,..tmp_cols]
            colnames(tmp_pe_dt) = "value"
            plot_effect1_dt.list[[i]] = tmp_pe_dt %>%
                        cbind(bfish_df[,c("year","species_cd",qeffectnames_1[i])],.) %>%
                        as.data.table(.) %>%
                        setnames(.,c("species_cd",qeffectnames_1[i]),c("species","variable")) %>%
                        .[,component:="1st"] %>%
                        .[,covariate:=qeffectnames_1[i]] %>%
        ...
        plot_effect2_dt.list = as.list(rep(NA,length(qeffectnames_2)))
        for(i in 1:length(qeffectnames_2))
        {
            tmp_cols = qeffectnames_2[i]
            tmp_pe_dt = qeffect_2_dt[,..tmp_cols]
            colnames(tmp_pe_dt) = "value"
            plot_effect2_dt.list[[i]] = tmp_pe_dt %>%
                        cbind(bfish_df[,c("year","species_cd",qeffectnames_2[i])],.) %>%
                        as.data.table(.) %>%
                        setnames(.,c("species_cd",qeffectnames_2[i]),c("species","variable")) %>%
                        .[,component:="2nd"] %>%
                        .[,covariate:=qeffectnames_2[i]] %>%
                        .[,.(component,species,covariate,variable,value)] %>%
                        unique(.) %>%
						.[,species:=factor(species,levels=c(target_species))] %>%
                        .[,value_link:=exp(value)]
