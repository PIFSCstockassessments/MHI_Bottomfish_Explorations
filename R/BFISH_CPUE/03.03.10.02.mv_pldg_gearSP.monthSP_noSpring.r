...
# specify (coarse) model configuration
    ...
    species = "mv"
    data_treatment = "05"
    catchability_covariates = "noSpring_gearSP.monthSP" # vanilla
...
# 1) bring in data
	...
	# remove early quarters to try and help estimation
		bfish_df = subset(bfish_df,!(month%in%c("02","03","07")))
    ...
    # define catchability section
        ...
        q1_formula = ~ gear_type:category + month:category
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		...
		if(length(fit_setup$parameter_estimates)==2)
		{
			...
			epsilon1_map = c(1,2,NA,3,4,5,NA)
			omega2_map = c(1,2,3,NA,4,NA,NA)
			epsilon2_map = c(1,2,NA,3,4,5,NA)
...
# predicted density
    ...
    # effects plots
	    ...
	    p = copy(plot_effect_dt) %>%
			...
			ylab("Effect") +
			xlab("Variable") +
			facet_wrap(covariate~species,scales="free",nrow=uniqueN(plot_effect_dt$covariate)) +
			geom_hline(yintercept=1,linetype="dashed") +
