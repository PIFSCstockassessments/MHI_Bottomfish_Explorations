...
# 2023/05/31
# Format BFISH camera data: Option 4
...
# bring in camera data
	...
	fwrite(unique(camera_dt[,.(DROP_CD)]),file=paste0(proj.dir,"Data/",data_flag,"04.camera_dt.all_lengths.drop_cd.csv"))
    ...
    camera_dt = camera_dt %>%
                ...
                .[,.(PSU=mode(PSU),SAMPLE_DATE=mode(SAMPLE_DATE),YEAR=mode(YEAR),MONTH=mode(MONTH),DAY=mode(DAY),JD=round(mean(JD)),YEAR_continuous=mean(YEAR_continuous),LUNAR_PHASE=mean(LUNAR_PHASE),DROP_TIME_HST=mean(DROP_TIME_HST),VESSEL=mode(VESSEL),OBS_LON=mean(OBS_LON),OBS_LAT=mean(OBS_LAT),OFFICIAL_DEPTH_M=mean(OFFICIAL_DEPTH_M),OFFICIAL_TEMP_C=mean(OFFICIAL_TEMP_C),ETCO=mean(ETCO),ETCA=mean(ETCA),PRSI=mean(PRSI),PRFI=mean(PRFI),PRZO=mean(PRZO),HYQU=mean(HYQU),APRU=mean(APRU),Island=mode(Island),STRATA=mode(STRATA),STRATA_2020=mode(STRATA_2020),Depth_MEDIAN_m=mean(Depth_MEDIAN_m),substrate=mode(substrate),slope=mode(slope),med_slp=mean(med_slp),med_acr=mean(med_acr),BS_pct_over_136j=mean(BS_pct_over_136j),pctHB=mean(pctHB),pctHS=mean(pctHS),depth_strata=mode(depth_strata),depth_strata_2020=mode(depth_strata_2020),complexity=mode(complexity),hardness=mode(hardness),SEASON=mode(SEASON),design_sampling_unit=mode(design_sampling_unit),.N),by=model_sampling_unit] %>%
                .[N==2] %>%
                .[,N:=NULL]


	# save formatted data
		save(camera_dt,file=paste0(proj.dir,"Data/",data_flag,"04.camera_dt.all_lengths.RData"))
