...
# specify (coarse) model configuration
    ...
    abundance_covariates = "depth.complexity" # vanilla
    lehi_filter = TRUE
...
# 1) bring in data
	...
	# define abundance section
		...
		ab1_formula = ~ bs(depth,df=3) + complexity
        ab2_formula = ~ 0

        continuous_ab_variables = c("depth","complexity")
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		...
		if(length(fit_setup$parameter_estimates)==2)
		{
			...
			omega2_map = c(1,2,3,NA,NA,NA,NA)
