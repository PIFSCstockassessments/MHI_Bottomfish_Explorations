...
# specify (coarse) model configuration
    data_flag = 2021
    link_function = "plgg" # logit-link delta-gamma; conventional model
    species = "mv"
    data_treatment = "05"
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
        ...
        # or are any variances blowing up
			...
			which(abs(fit_setup$ParHat$L_omega1_z)<1e-2|abs(fit_setup$ParHat$L_omega1_z)>1.5e1)
			which(abs(fit_setup$ParHat$L_epsilon1_z)<1e-2|abs(fit_setup$ParHat$L_epsilon1_z)>1.5e1)
			which(abs(fit_setup$ParHat$L_omega2_z)<1e-2|abs(fit_setup$ParHat$L_omega2_z)>1.5e1)
			which(abs(fit_setup$ParHat$L_epsilon2_z)<1e-2|abs(fit_setup$ParHat$L_epsilon2_z)>1.5e1)
