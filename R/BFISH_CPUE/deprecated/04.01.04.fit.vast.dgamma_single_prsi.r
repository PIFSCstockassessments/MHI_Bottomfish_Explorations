...
# 3) Fit single species (prsi) model using delta-Gamma distribution
...
# set working directory
	...
	working_dir = paste0(proj.dir,"VAST/model_runs/dgamma_single_prsi/")
...
# 1) bring in data
	...
	# subset to species
	bfish_df = bfish_combined_long_dt %>% .[species_cd %in% c("prsi")] %>% as.data.frame(.)
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		fit_setup = fit_model( settings=settings,
	    	  					...
	    	  					c_i = as.numeric(factor(bfish_df[,'species_cd'],levels=c("prsi")))-1,
	    	  					category_names=c("prsi"),
	    	  					covariate_data = NULL,
		...
		settings$FieldConfig = matrix( c("IID","IID","IID",0,"IID","IID"), ncol=2, nrow=3, dimnames=list(c("Omega","Epsilon","Beta"),c("Component_1","Component_2")) )
