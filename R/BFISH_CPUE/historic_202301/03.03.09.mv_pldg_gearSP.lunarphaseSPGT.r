...
# specify (coarse) model configuration
    ...
    species = "mv"
    data_treatment = "05"
    catchability_covariates = "gearSP.lunarphaseSPGT" # vanilla
...
# 1) bring in data
    ...
    # define catchability section
        q_data = bfish_df[,c("year","species_cd","gear_type","lunar_phase")]
        ...
        q1_formula = ~ gear_type:category + lunar_phase:category:gear_type
        q2_formula = ~ gear_type:category

        continuous_q_variables = c("lunar_phase")
    ...
    # effects plots
        ...
        if(length(continuous_q_variables)>0)
        {
            ...
            for(i in 1:length(factor_effects_1))
            {
                ...
                colnames(tmp_pe_dt) = "value"
                plot_effect1_dt.list[[i]] = tmp_pe_dt %>%
                            cbind(bfish_df[,c("year","species_cd","gear_type",factor_effects_1[i])],.) %>%
                            as.data.table(.) %>%
                            setnames(.,c("species_cd",factor_effects_1[i]),c("species","variable")) %>%
                            .[,component:="1st"] %>%
                            .[,covariate:=factor_effects_1[i]] %>%
                            .[,.(component,gear_type,species,covariate,variable,value)] %>%
                            unique(.) %>%
                            .[,species:=factor(species,levels=c(target_species))] %>%
                            .[,value_link:=exp(value)]
            ...
            for(i in 1:length(factor_effects_2))
            {
                ...
                colnames(tmp_pe_dt) = "value"
                plot_effect2_dt.list[[i]] = tmp_pe_dt %>%
                            cbind(bfish_df[,c("year","species_cd","gear_type",factor_effects_2[i])],.) %>%
                            as.data.table(.) %>%
                            setnames(.,c("species_cd",factor_effects_2[i]),c("species","variable")) %>%
                            .[,component:="2nd"] %>%
                            .[,covariate:=factor_effects_2[i]] %>%
			...
			facet_grid(covariate~species,scales="free_x") +
			geom_hline(yintercept=1,linetype="dashed") +
			geom_path(aes(x=variable,y=value_link,color=gear_type)) +
            theme_few(base_size=20) +
            theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
            viridis::scale_fill_viridis("Gear",begin = 0.1,end = 0.8,direction = 1,option = "H")
