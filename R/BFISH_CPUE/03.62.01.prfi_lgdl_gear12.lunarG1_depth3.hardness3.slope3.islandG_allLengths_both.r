...
# specify (coarse) model configuration
    ...
    species = "prfi"
    data_treatment = "01"
    catchability_covariates = "gear12.lunarG1" # vanilla
...
# 1) bring in data
    ...
    # define catchability section
		...
		if(gears == "both")
		{
			q1_formula = ~ gear_type + gear_type:lunarphase_sc
        	q2_formula = ~ gear_type
		} else {
			q1_formula = ~ 0 + gear_type:lunarphase_sc
        	q2_formula = ~ 0
		}


        continuous_q_variables = c("depth","lunarphase_sc","month_sc")
