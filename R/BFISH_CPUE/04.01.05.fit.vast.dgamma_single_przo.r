...
# 3) Fit single species (przo) model using delta-Gamma distribution
...
# set working directory
	...
	working_dir = paste0(proj.dir,"VAST/model_runs/dgamma_single_przo/")
...
# 1) bring in data
	...
	# subset to species
	bfish_df = bfish_combined_long_dt %>% .[species_cd %in% c("przo")] %>% as.data.frame(.)
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		fit_setup = fit_model( settings=settings,
	    	  					...
	    	  					c_i = as.numeric(factor(bfish_df[,'species_cd'],levels=c("przo")))-1,
	    	  					category_names=c("przo"),
	    	  					covariate_data = NULL,
