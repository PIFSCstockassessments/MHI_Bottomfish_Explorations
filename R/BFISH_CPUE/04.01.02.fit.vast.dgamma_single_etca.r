...
# 3) Fit single species (etca) model using delta-Gamma distribution
...
# set working directory
	...
	working_dir = paste0(proj.dir,"VAST/model_runs/dgamma_single_etca/")
...
# 1) bring in data
	...
	# subset to species
	bfish_df = bfish_combined_long_dt %>% .[species_cd %in% c("etca")] %>% as.data.frame(.)
...
# Random effects: spatial + spatiotemporal
	...
	# set-up model
		fit_setup = fit_model( settings=settings,
	    	  					...
	    	  					c_i = as.numeric(factor(bfish_df[,'species_cd'],levels=c("etca")))-1,
	    	  					category_names=c("etca"),
	    	  					covariate_data = NULL,
