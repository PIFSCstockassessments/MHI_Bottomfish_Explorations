...
post.rel.mortality <- 0 # post-release mortality of released crabs/fish, set to 0 if none
...
mm <- if(Model == 4){
    rlnorm(nrow(out), log(m), shape.CV)
  } else{
    rep(1, nrow(out))
  }
ppsi <- if(psi.dist == "beta"){
    rbeta(nrow(out), (psi.pr[1]), psi.pr[2])
  } else {
    rlnorm(nrow(out), log(psi.prior[1]), psi.prior[2])
  }
qq1 <- rep(0, nrow(out))
qq2 <- rep(0, nrow(out))
...
png(
  file = paste0(output.dir, "/LogResiduals_", assessment, "_", Scenario, ".png"), width = 6, height = 10,
  res = 1080, units = "in"
)
par(Par)
for (i in 1:n.indices) {
...
png(
  file = paste0(output.dir, "/ProcDev_", assessment, "_", Scenario, ".png"), width = 6, height = 4,
  res = 800, units = "in"
)
par(Par)
...
png(
  file = paste0(output.dir, "/Biomass_", assessment, "_", Scenario, ".png"), width = 5, height = 3.5,
  res = 720, units = "in"
)
par(Par)
ylim <- c(0, max(mu.B))
...
png(
  file = paste0(output.dir, "/Fig11_TrendMSY_", assessment, "_", Scenario, ".png"), width = 5, height = 7,
  res = 720, units = "in"
)
par(Par)
## H/Hmsy
...
if (SP.plot != "phase") {
  Par <- list(mfrow = c(1, 1), mar = c(3.5, 3.5, 0.1, 0.1), mgp = c(2., 0.5, 0), tck = -0.02, cex = 0.8)
  png(
    file = paste0(output.dir, "/SP_", assessment, "_", Scenario, ".png"), width = 6, height = 6,
    res = 800, units = "in"
  )
  par(Par)
...
if (SP.plot == "phase") {
  ...
  Par <- list(mfrow = c(1, 1), mar = c(3.5, 3.5, 0.1, 0.1), mgp = c(2., 0.5, 0), tck = -0.02, cex = 0.8)
  png(
    file = paste0(output.dir, "/SPphase_", assessment, "_", Scenario, ".png"), width = 6, height = 6,
    res = 800, units = "in"
  )
  par(Par)
...
if (Biplot == TRUE) {
  ...
  png(
    file = paste0(output.dir, "/Biplot_", assessment, "_", Scenario, ".png"), width = 6, height = 6,
    res = 800, units = "in"
  )
  par(Par)
  ...
  sel.yr <- c(1, round(quantile(1:N, 0.7), 0), N)
  points(mu.f[2, sel.yr], mu.b[2, sel.yr],
    col =
      1, pch = c(22, 21, 24), bg = "white", cex = 1.9
  )
...
if (Projection == TRUE) {
  Par <- list(mfrow = c(1, 1), mar = c(3.5, 3.5, 0.1, 0.1), mgp = c(2., 0.5, 0), tck = -0.02, cex = 0.8)
  png(
    file = paste0(output.dir, "/Fig21_Projections_", assessment, "_", Scenario, ".png"), width = 7, height = 5,
    res = 800, units = "in"
  )
  par(Par)
...
if (KOBE.plot == TRUE) {
  ...
  Par <- list(mfrow = c(1, 1), mar = c(3.5, 3.5, 0.1, 0.1), mgp = c(2., 0.5, 0), tck = -0.02, cex = 0.8)
  png(
    file = paste0(output.dir, "/Fig12_Kobe_", assessment, "_", Scenario, ".png"), width = 6, height = 6,
    res = 800, units = "in"
  )
  par(Par)
  ...
  lines(mu.b[2, ], mu.f[2, ], lty = 1, lwd = 1.)
  sel.yr <- c(1, round(quantile(1:N, 0.7), 0), N)
  points(mu.b[2, sel.yr], mu.f[2, sel.yr],
    col =
      1, pch = c(22, 21, 24), bg = "white", cex = 1.9
  )
