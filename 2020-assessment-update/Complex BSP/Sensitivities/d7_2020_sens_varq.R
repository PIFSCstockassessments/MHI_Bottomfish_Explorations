...
addname <- 'sens_varq'  ##<--------name of model---------- # change accordingly
...
cat("
    ...
    # Uniform prior on log-scale for q[1]'s, which follow random walk.
    # Variation around q[t>1]'s on log scale follow normal with CV=0.5
    #(P4)#########################################################
    log_q1[1] ~ dunif(logq_lo, logq_hi) 
    log_q2[1] ~ dunif(logq_lo, logq_hi)
    q1[1] <- exp(log_q1[1])
    q2[1] <- exp(log_q2[1])
    q_Prior_Precision <- 1.0/log(1.0+CV_q*CV_q)
    ...
    #(L2)##########################################################
    CPUE_mean[(NCPUE_S1_1+NCPUE_S1_MISS+1)] <- log(q1[(NCPUE_S1_1+NCPUE_S1_MISS+1)]*K*P[(NCPUE_S1_1+NCPUE_S1_MISS+1)])
    Precision_CPUE[(NCPUE_S1_1+NCPUE_S1_MISS+1)] <- itau2_1/(CPUE_S1_REL_CV[(NCPUE_S1_1+NCPUE_S1_MISS+1)]*CPUE_S1_REL_CV[(NCPUE_S1_1+NCPUE_S1_MISS+1)])
    CPUE_S1[(NCPUE_S1_1+NCPUE_S1_MISS+1)] ~ dlnorm(CPUE_mean[(NCPUE_S1_1+NCPUE_S1_MISS+1)],Precision_CPUE[(NCPUE_S1_1+NCPUE_S1_MISS+1)])
    LOG_RESID1[(NCPUE_S1_1+NCPUE_S1_MISS+1)] <- log(CPUE_S1[(NCPUE_S1_1+NCPUE_S1_MISS+1)]) - log(q1[(NCPUE_S1_1+NCPUE_S1_MISS+1)]*K*P[(NCPUE_S1_1+NCPUE_S1_MISS+1)])
    
    for (i in (NCPUE_S1_1+NCPUE_S1_MISS+1+1):NCPUE_S1_2) {
    log_q1[i] ~ dnorm(log_q1[i-1],q_Prior_Precision)I(-11.51293,11.51293)
    q1[i] <- exp(log_q1[i])
    CPUE_mean[i] <- log(q1[i]*K*P[i])
    Precision_CPUE[i] <- itau2_1/(CPUE_S1_REL_CV[i]*CPUE_S1_REL_CV[i])
    CPUE_S1[i] ~ dlnorm(CPUE_mean[i],Precision_CPUE[i])
    LOG_RESID1[i] <- log(CPUE_S1[i]) - log(q1[i]*K*P[i])
    }
    
    
    # deep7 bottomfish CPUE lLIKELIHOOD, 2003-2019 P[(NCPUE_S1_2+1):NCPUE_S1_3]
    #(L3)##########################################################
    CPUE_mean2[NCPUE_S1_2] <- log(q2[1]*K*P[NCPUE_S1_2])
    Precision_CPUE2[NCPUE_S1_2] <- itau2_2/(CPUE_S2_REL_CV[NCPUE_S1_2]*CPUE_S2_REL_CV[NCPUE_S1_2])
    CPUE_S2[NCPUE_S1_2] ~ dlnorm(CPUE_mean2[NCPUE_S1_2],Precision_CPUE2[NCPUE_S1_2])
    LOG_RESID2[NCPUE_S1_2] <- log(CPUE_S2[NCPUE_S1_2]) - log(q2[1]*K*P[NCPUE_S1_2])
    
    for (i in (NCPUE_S1_2+1):NCPUE_S1_3) {
    log_q2[i-NCPUE_S1_2+1] ~ dnorm(log_q2[i-1-NCPUE_S1_2+1],q_Prior_Precision)I(-11.51293,11.51293)
    q2[i-NCPUE_S1_2+1] <- exp(log_q2[i-NCPUE_S1_2+1])
    CPUE_mean2[i] <- log(q2[i-NCPUE_S1_2+1]*K*P[i])
    Precision_CPUE2[i] <- itau2_2/(CPUE_S2_REL_CV[i]*CPUE_S2_REL_CV[i])
    CPUE_S2[i] ~ dlnorm(CPUE_mean2[i],Precision_CPUE2[i])
    LOG_RESID2[i] <- log(CPUE_S2[i]) - log(q2[i-NCPUE_S1_2+1]*K*P[i])
    }
   ...
   #need to add P2020
    ...
    PMSY <- BMSY/K
    FMSY <- -log(1-HMSY)
    CPUE_MSY <- q2[NTIME-NCPUE_S1_2+1]*BMSY
...
inits <- list(    # create inits list of functions
  ...
  list(
    ...
    log_q1=rep(log(10.0),NCPUE_S1_2),
    log_q2=rep(log(10.0),NTIME-NCPUE_S1_2+1),
    rad=27.6,
