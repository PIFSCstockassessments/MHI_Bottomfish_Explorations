...
d7_2020_sens = function(data,survey_data,filename,K_Prior_avg,r_Prior_avg,
                        P1_Prior_avg,Mscale,tau2_scale,sigma2_scale,
                        nat_mort,unrep_c_err,dir_err,sweight,
                        radius_Prior_avg,radius_CV,initC,unrep2019){
  

#DATA = read.csv("D:\\Stock assessments\\Hawaii\\MHI_bottomfish_2017\\model code\\SPM code\\d7_2017_data_REMLF.csv",header=T)
DATA = data #read.csv("C:\\Users\\John.Syslo\\Documents\\2020_Deep_7\\Complex BSP\\data.csv",header=T)
#DATA = read.csv("C:\\Users\\John.Syslo\\Documents\\2020 Deep 7\\Complex BSP\\data_early_old.csv",header=T)
head(DATA)
DATA=DATA[-1,]

Survey_data = survey_data #read.csv("C:\\Users\\John.Syslo\\Documents\\2020_Deep_7\\Complex BSP\\Survey_data.csv",header=T)

addname <- filename #'d7_2020_base_proper_rad_500'  ##<--------name of model---------- # change accordingly
#src.dir <- paste('D:\\Stock assessments\\Hawaii\\MHI_bottomfish_2017\\model code\\SPM code\\',addname,"\\",sep="") # Change accordingly
src.dir <- paste('C:\\Users\\John.Syslo\\Documents\\2020_Deep_7\\Complex BSP\\Sensitivity\\',addname,"\\",sep="")
...
s_lambda <- sweight #initial weighting on sd of survey estimate
...
q_lo <- 0.0001    #reduce by 000 to avoid undefined real (r scenario 0.15)
q_hi <- 10000     #reduce by 000 to avoid undefined real (unrep catch 5)

Target_rad_Prior_avg <-radius_Prior_avg
CV_rad <- radius_CV

LB <- 1-unrep_c_err+(1-unrep_c_err)*(dir_err)
UB <- 1+unrep_c_err+(1-unrep_c_err)*(dir_err)
...
cat("
    ...
    C2019lo <- LB*unrep2019 + 0.177051  #last quantity is reported
    C2019hi <- UB*unrep2019 + 0.177051  #last quantity is reported
