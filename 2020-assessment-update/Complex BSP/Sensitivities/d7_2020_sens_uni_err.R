...
addname <- 'sens_uniferr'  ##<--------name of model---------- # change accordingly
...
cat("
    ...
    #(P4)##########################################################
    q1 ~ dunif(q_lo, q_hi)
    q2 ~ dunif(0.001, 1000) #updated for this sensitivity
    ...
    q3 <- 250000/(rad*rad*3.14159)                                 #rlnorm for 27.6 gives (10.9 and 69.5 for 95% CI)
      ...
      # Uniform prior for process error standard deviation, sigma
    #(P5)##########################################################
    sigma ~ dunif(0,100)
    sigma2 <- sigma*sigma
    isigma2 <- 1/sigma2

       # Uniform prior for observation error standard deviation, tau
    #(P6)##########################################################
    tau_1 ~ dunif(0,100)
    tau2_1 <- tau_1*tau_1
    itau2_1 <-1/tau2_1
