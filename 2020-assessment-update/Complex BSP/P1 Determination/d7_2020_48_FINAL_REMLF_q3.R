...
DATA = read.csv("C:\\Users\\John.Syslo\\Documents\\2020 Deep 7\\Complex BSP\\data.csv",header=T)
head(DATA)
DATA=DATA[-1,]

Survey_data = read.csv("C:\\Users\\John.Syslo\\Documents\\2020 Deep 7\\Complex BSP\\Survey_data.csv",header=T)

addname <- 'd7_2020_base_q3_550_250'  ##<--------name of model---------- # change accordingly
#src.dir <- paste('D:\\Stock assessments\\Hawaii\\MHI_bottomfish_2017\\model code\\SPM code\\',addname,"\\",sep="") # Change accordingly
src.dir <- paste('C:\\Users\\John.Syslo\\Documents\\2020 Deep 7\\',addname,"\\",sep="")
...
nt <- 20     # Thinning rate
ni <- 550000 # Number of total iterations per chain, including burn-in = multiply # of iterations desired by nt. 130,000 runs per chain thinned by 5 starting at 10,000
nb <- 250000 #round(ni*(1/10)) # Number of draws to discard as burn in; discard 10,000 
...
BioYR <- BIOSV/1000000*2.20462/(25892*194.89)
s_eta2 <- (SE_SV/1000000*2.20462/(25892*194.89))^2
