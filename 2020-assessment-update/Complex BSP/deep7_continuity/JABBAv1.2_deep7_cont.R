...
##
## Updated as of Nov 5, 2018 to include proper accounting of Harvest Control
## Rule for territorial assessment
## Updated Feb 12 2019 to account for John's changes for report plotting
## ><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><><><
...
#><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>><>
if (exists("igamma") == FALSE) igamma <- c(0.2, 0.1) # Generic process error prior  WAS 4,0.01
if (exists("BmsyK") == FALSE) BmsyK <- 0.4 # JABBA default for Pella model
if (exists("Model") == FALSE) {
  model <- 1
  Mod.names <- c("Schaefer")
} # Run Schaefer if model is not specified
...
if (exists("q_bounds") == FALSE) q_bounds <- c(10^-10, 10) # Defines lower and upper bounds for q
...
## create a vector of cpue values:
conv.cpue <- as.numeric(rbind(matrix(rep(NA, (styr.I - 1) * n.indices), styr.I - 1, n.indices), 
                              as.matrix(cpue[, -1])))
## turn df into matrix (nrow = nyears.I, ncol = n.indices):
...
if (SE.I == FALSE) {
  ...
  se <- cpue
  conv.se <- as.numeric(rbind(matrix(rep(NA, (styr.I - 1) * n.indices), styr.I - 1, n.indices), 
                              as.matrix(cpue[, -1])))
...
if (Catch.CV == FALSE) {
  ...
  conv.catch <- as.numeric(rbind(matrix(rep(NA, (styr.I - 1) * n.catches), styr.I - 1, n.catches), 
                                 as.matrix(catch[, -1])))
  ...
  conv.catch.cv <- as.numeric(rbind(matrix(rep(NA, (styr.I - 1) * n.catches), styr.I - 1, n.catches), 
                                    as.matrix(cv.c[, -1]))) # assumes we have a cv for every catch
...
png(
  file = paste0(input.dir, "/Catches_", assessment, ".png"), width = 7, height = 5,
  res = 200, units = "in"
)
par(Par)
...
# Index color palette
#---------------------
jabba.colors <- as.character(rep(c(
  nmfs_palette("regional web")(5)
), 2))
#--------------------
# Set seed
#--------------------
if (Reproduce.seed == FALSE) {
  set.seed(ceiling(runif(1, min = 0, max = 1e6)))
} else {
  set.seed(123)
}
...
if (CPUE.plot == TRUE) {
  ...
  if (n.indices > 1) {
    for (i in 2:n.indices) {
      q.init[i] <- mean(mCPUE[, i], na.rm = TRUE) / mean(mCPUE[, 1], na.rm = TRUE)
    }
  }
  # Bundle data
  ...
  # Initial values
  inits <- function() {
    list(isigma2.est = runif(1, 20, 100), itau2 = runif(1, 80, 200), mean.r = rnorm(1), iq = 1 / q.init)
  }
  ...
  png(
    file = paste0(input.dir, "/CPUE_", assessment, "_", Scenario, ".png"), width = 5, height = 3.5,
    res = 200, units = "in"
  )
  par(Par)
  u.ylim <- NULL
  for (i in 1:n.indices) {
    u.ylim <- c(u.ylim, exp(log(mCPUE[, i] / q.adj[i]) + 1.96 * sqrt(mSE2[, i])))
  }
...
if (r.dist == "range") {
  ...
  if (r.prior == "High") {
    start.r <- c(0.6, 1.5)
  } else if (r.prior == "Medium") {
    start.r <- c(0.2, 0.8)
  } else if (r.prior == "Low") {
    start.r <- c(0.05, 0.5)
  } else { # i.e. res== "Very low"
    start.r <- c(0.015, 0.1)
  }
...
png(
  file = paste0(input.dir, "/Priors_", assessment, "_", Scenario, ".png"), width = 9, height = 3,
  res = 200, units = "in"
)
par(Par)
...
if (psi.dist == "beta") {
  psi.pr <- get_beta(mu = psi.prior[1], CV = psi.prior[2], Min = 0, Prior = paste0("Prior B(", years[1], ")/K"))
} else {
...
png(
  file = paste0(input.dir, "/Production", assessment, "_", Scenario, ".png"), width = 6, height = 5,
  res = 200, units = "in"
)
par(Par)
...
if (Catch.CV == FALSE) {
  inits <- function() {
    list(K = rlnorm(1, log.K, 0.3), q = runif(nq, 0.005, 0.5), isigma2.est = runif(1, 20, 100), itau2 = runif(nvar, 80, 200))
  }
} else {
  # inits <- function(){list(K=4*55/0.46,r=0.46,m=2,rCatch = TC, psi=0.75, q = 5, isigma2.est=100, itau2=100)}
  inits <- function() {
    list(K = rlnorm(1, log(4 * 55 / 0.46), 0.5), r = rlnorm(1, log(0.46), 0.5), m = rlnorm(1, log(2), 0.5), rCatch = TC, psi = rlnorm(1, log(0.75), 0.5), q = runif(1, 0.0000000001, 10), isigma2.est = rgamma(1, 0.2, 0.1), itau2 = rgamma(1, 0.2, 0.1))
  }
}
...
if (init.values == TRUE) {
  inits <- function() {
    list(K = K.init, r = r.init, q = q.init, isigma2.est = runif(1, 20, 100), itau2 = runif(nvar, 80, 200))
  }
}
...
if (Catch.CV == FALSE) {
  surplus.dat <- list(
    N = n.years, TC = TC, I = CPUE, SE2 = se2, mu.m = m, r.pr = r.pr, psi.pr = psi.pr, K.pr = K.pr,
    ...
    nTAC = nTAC, pyrs = pyrs, TAC = TAC, igamma = igamma, stI = stI, TACint = TACint, P_bound = P_bound, proc.pen = 0, K.pen = 0,
    obs.pen = rep(0, nvar), q_bounds = q_bounds, sigmaobs_bound = sigmaobs_bound, sigmaproc_bound = sigmaproc_bound, K_bounds = K_bounds
  )
} else {
  surplus.dat <- list(
    N = n.years, TC = TC, catch.cv = catch.cv, I = CPUE, SE2 = se2, mu.m = m, r.pr = r.pr, psi.pr = psi.pr, K.pr = K.pr,
    nq = nq, nI = nI, nvar = nvar, sigma.fixed = ifelse(sigma.proc == TRUE, 0, sigma.proc),
    sets.var = sets.var, sets.q = sets.q, pen.bk = rep(0, n.years), Plim = Plim, slope.HS = slope.HS,
    nTAC = nTAC, pyrs = pyrs, TAC = TAC, igamma = igamma, stI = stI, TACint = TACint, P_bound = P_bound, proc.pen = 0, K.pen = 0,
    obs.pen = rep(0, nvar), q_bounds = q_bounds, sigmaobs_bound = sigmaobs_bound, sigmaproc_bound = sigmaproc_bound, K_bounds = K_bounds
...
if (Catch.CV == FALSE) {
  params <- c("K", "r", "q", "psi", "sigma2", "tau2", "m", "Hmsy", "SBmsy", "MSY", "BtoBmsy", "HtoHmsy", "Overfishing_ind", "CPUE", "Proc.Dev", "P", "SB", "H", "prP", "prBtoBmsy", "prHtoHmsy", "TOE")
} else {
  params <- c("K", "r", "q", "psi", "sigma2", "tau2", "m", "Hmsy", "SBmsy", "MSY", "BtoBmsy", "HtoHmsy", "Overfishing_ind", "CPUE", "Proc.Dev", "P", "SB", "H", "prP", "prBtoBmsy", "prHtoHmsy", "TOE")
}
...
# Process equation
    ...
    log(max(P[t-1] +  r/(m-1)*P[t-1]*(1-pow(P[t-1],m-1))*P[t-1]*slope.HS - rCatch[t-1]/K,0.005)))
...
max.acK <- pmax(abs(autocorr1$K$acf), abs(autocorr1$K$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.acr <- pmax(abs(autocorr1$r$acf), abs(autocorr2$r$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.acq <- pmax(abs(autocorr1$q$acf), abs(autocorr2$q$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.acpsi <- pmax(abs(autocorr1$psi$acf), abs(autocorr2$psi$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.acsigma2 <- pmax(abs(autocorr1$sigma2$acf), abs(autocorr2$sigma2$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.actau2 <- pmax(abs(autocorr1$tau2$acf), abs(autocorr2$tau2$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
max.acm <- pmax(abs(autocorr1$m$acf), abs(autocorr2$m$acf))[c(2, 6)] # max lag1 and lag5 autocorrelation across chains
...
results <- data.frame(Median = results[, 2], LCI = results[, 1], UCI = results[, 3], Geweke.p1 = round(pvalues1, 3), Geweke.p2 = round(pvalues2, 3), Heidel.p1 = round(heidle1[, 3], 3), Heidel.p2 = round(heidle2[, 3], 3), Heidel.it1 = heidle1[, 2], Heidel.it2 = heidle2[, 2], Heidel.hw.pass1 = heidle1[, 4] == 1, Heidel.hw.pass2 = heidle2[, 4] == 1, lag1 = max.ac[, 1], lag5 = max.ac[, 2]) # added all heidleberger and welch diagnostics 10-25-2018
...
source(paste0(JABBA.file, "/plot_JABBA_report.R"))
